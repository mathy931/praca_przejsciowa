...
class RepeatActionAndMaxFrame(gym.Wrapper):
    ...
    def reset(self):
        ...
        for _ in range(no_ops):
            _,_,done,_ = self.env.step(0) #_ pomija nam obs,reward, info bo powinno byc tak  (obs, reward, done, info)
            if done:
                self.env.reset()
                
        if self.fire_first:
            assert self.env.unwrapped.get_action_meaning()[1] == 'FIRE'       #assert sprawdza warunek i wyrzuca od razu error
